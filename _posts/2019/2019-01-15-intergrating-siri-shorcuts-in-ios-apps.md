...
In Apple's WWDC 2018, Apple introduced a new feature called Siri Shorcut for iOS 12 which allows to define and access the frequently used tasks of the application using Siri. 

#### References
Refer [here](https://support.apple.com/en-us/HT209055) for more details about Siri Shortcuts.

In this tutorial, we will use an [PincodeSwift iOS Application](https://mobilefirstplatform.ibmcloud.com/tutorials/en/foundation/8.0/authentication-and-security/credentials-validation/ios/) and integrate Siri Shorcuts capability into the app.  The shortcut can be invoked using a voice phrase and will fetch the account balance using a resource adapter.  Follow the steps listed in the tutorial to understand how to create a shortcut and enable security using Mobile Foundation server. 
...
![Intents Extension]({{site.baseurl}}/assets/blog/2019-01-15-intergrating-siri-shorcuts-in-ios-apps/intents-extension.png)
	...
	This will add an new Intent Extenions target to the iOS Application.

#### Add SiriKit Intent Defnition

- In BalanceIntent Folder, Right Click and Click on New File > Add SiriKit Intent Defnition File. Add it to `BalanceIntent` and `PincodeSwift` targets and Save it as `GetBalance`
...
- Open the the GetBalance Intent Definition file, Press ➕ symbol to add new intent as `GetBalance`. Fill the details as shown in the below.
![Custom Intent]({{site.baseurl}}/assets/blog/2019-01-15-intergrating-siri-shorcuts-in-ios-apps/custom-intent.png)

	Here we have selected Intent Category as `View` since we are viewing the information and defined the title's & description of the Shorcut. I have defined bankName as a parameter which is required in case if the developer wishes to create shorcuts to view balance for multiple banks. We have checked background execution with which shorcut can be executed without having to run the actual application.

- Define the responses for the Intent as shown in the below.
	...
	Here we have defined success and two failure responses where default error response can be used in case if we don't have errorInfo else custom error response which can be used in case if we have error info.


#### Add Intent Handler

- Add native iOS Mobile Foundation Platform SDK in the `GetBalance` target. This can be done by using cocoapod or by manually. You can refer the documentation [here](https://mobilefirstplatform.ibmcloud.com/tutorials/en/foundation/8.0/application-development/sdk/ios/) for adding Mobile Foundation SDK.
...
- Create a new swift file with a name GetBalanceIntentHandler.swift and Replace the content with the following.
	...
	Here we are making a `WLResourceRequest` adapter call to Mobile Foundation Server to retreive balance and sending back success or failure responses to the user.
...
- Replace the content of `ViewController.swift` with the following.
	...
	```
	where `donateInteraction` method donates the View Balance shorcut to the Siri when the app is installed on the device which makes the shorcut available to the user under Siri Suggested Shorcuts tab.
...
That's all, You are done with adding Siri Shorcut capbility to PincodeSwift application.
...
In the case of a Banking shortcut, Security is one of the major concern and this section details how you can enable security in the shortcuts. Since shortcuts are not designed to accept any user inputs. However there are multiple ways to handle the security in shorcuts. I will not go into great details for the scope of this article but here is one possible way to add security in shorcut:
...
### Source Code
The source code of the application is uploaded in [this](https://github.com/MobileFirst-Platform-Developer-Center/PincodeSwiftSiriShorcut) GitHub repository.

	
### Conclusion

Siri Shorcut is a very nice feature with great user experience. Shorcuts are meant to improve the user experience, bringing useful data to the user at a quick glance and This is used generally for actions which are used repeatedly. For example use an Application to check balance or get loyalty points. Such user repeated actions can be exposed as a shortcut and will defnitely helps to improve the user experience of the application.