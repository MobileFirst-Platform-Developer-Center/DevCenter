...
* With new Iphone X release from apple there is a new display which has notch at the top . The cordova / hybrid apps are affected by this new notch introduced in Iphone X .It is an issue with apple or cordova , and it is not addressed/fixed by apple or cordova .Hence MobileFirst hybrid apps will have rectangular display as it was before . Here is a related post that talks about the same issue and a work around for it [here](https://stackoverflow.com/questions/46232812/cordova-app-not-displaying-correctly-on-iphone-x-simulator) . Meanwhile we will keep watching for an update from cordova/apple if they provide a fix for this issue in their future update / releases, and accordingly we will adapt those changes into MobileFirst v7.1 .
