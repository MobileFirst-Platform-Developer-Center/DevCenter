...
Using `WLResourceRequest`'s `send` method with a custom `WLHttpResponseListener` is not currently supported when using a gateway.

If your code implementation uses `WLHttpResponseListener`, it is recommended to use `WLClient.connect` or `WLAuthorizationManager.obtainAuthorizationHeader` before making the request, to insure that the challenge is handled.
