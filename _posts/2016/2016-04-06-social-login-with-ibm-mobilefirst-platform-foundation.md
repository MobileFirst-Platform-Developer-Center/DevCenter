...
## Introduction
[Social Login](https://www.wikiwand.com/en/Social_login) has become a common authentication method in many mobile apps.  The app owners and the users understand the big value it gives them.
Users can now sign in to apps with their favorite social platform like Facebook or Google.
This method makes life easier for the users, and also can give the app owner ability to leverage the information from the social platform (e.g. profile picture, users friends, feed etc). App owner can maximize the user engagements with such capabilities.  

This blog post is taking into a consideration that you have basic knowledge about IBM MobileFirst Platform Foundation 8.0 Beta authentication and security checks, if not please refer to [this chapter in dev center](https://mobilefirstplatform.ibmcloud.com/tutorials/en/foundation/8.0/authentication-and-security/).
...
## Running the demo
In order to run this demo in your environment please see the following instructions [mfpdev github repository](https://github.com/mfpdev/mfp-advanced-adapters-samples/tree/development/custom-security-checks/social-app-samples/SocialLoginSample)

## Let's see the big picture
![login flow]({{site.baseurl}}/assets/blog/2016-04-06-social-login-with-ibm-mobilefirst-platform-foundation/login-flow.png)

The diagram above shows the login flow (described with Google, and relevant to Facebook or other social provider as well), meaning that the trigger to call the social provider is initiated by client.
...
4. The MFP SDK send the credentials and scope to the Authorization Server API.
...
8. The Authorization Server API return the authenticated user data to the MFP SDK.
9. The MFP SDK is calling to the handleSuccess method in the challenge handler with the authenticated used data.
10. The MFP SDK is calling to login success callback on the app.
...
Calling to protected resource directly without call to preemptive login can be done as well, which means you don't need to call to preemptive login if you don't really need it in your app business logic.   
