...
de
origem. Mas o proxy está configurado para encaminhar solicitações de API REST para o arquivo WAR do serviço de análise de dados. Configure essa propriedade se você instalou seus arquivos WAR em servidores de aplicativos separados.  | Nenhuma |
...
Como o Elasticsearch está integrado no {{site.data.keys.mf_analytics_server }} e é responsável por participar no cluster, não use os recursos do servidor de aplicativos para definir o comportamento do cluster. Você não deseja criar um farm do WebSphere Application Server Liberty, por exemplo. Confie no tempo de execução de Elasticsearch subjacente para participar no cluster. No entanto, você deve configurá-lo corretamente. 
