...
개발 프로세스를 용이하게 하고 가속화하기 위해 {{ site.data.keys.product }}에서는 `SecurityCheck` 인터페이스의 기본 추상 구현을 제공합니다. 또한 제공된 기본 보안 검사 클래스 각각에 대한 보충적 샘플 보안 검사 구성 클래스뿐만 아니라 `SecurityCheckConfiguration` 인터페이스의 기본 abstract 구현이 제공됩니다(`SecurityCheckConfigurationBase`). 사용자의 개발 요구사항에 최적화된 기본 보안 검사 구현(및 관련된 샘플 구성)을 사용하여 시작하고, 필요에 따라 구현을 확장 및 수정하십시오.
