...
5. Apache Cordova를 사용하여 개발되는 애플리케이션에 직접 업데이트를 사용하려는 경우 앱을 빌드하는 데 사용한 Cordova 플러그인 버전을 아카이브하십시오.
    ...
    직접 업데이트를 사용하여 네이티브 코드를 업데이트할 수 없습니다. Cordova 프로젝트에서 네이티브 라이브러리 또는 빌드 도구 중 하나를 변경하고 해당 파일을 {{ site.data.keys.mf_server }}에 업로드한 경우 서버에서 차이점을 발견하고 클라이언트 애플리케이션의 업데이트를 전송하지 않습니다. 네이티브 라이브러리의 변경사항에는 다른 Cordova 버전, 최신 Cordova iOS 플러그인 또는 원래 애플리케이션을 빌드하는 데 사용된 것보다 최신인 mfpdev 플러그인 수정팩이 포함될 수 있습니다.
