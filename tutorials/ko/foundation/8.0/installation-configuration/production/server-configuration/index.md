...
2. IBM Java SDK를 사용하는 경우 **server.xml** 파일을 편집하십시오.
    ...
    * `sslProtocol="SSL_TLSv2"` 속성을 기존 모든 `<ssl>` 요소에 추가하십시오.
...
보안 역할을 구성하려면 **server.xml** 파일을 편집해야 합니다. 각 `<application>` 요소의 `<application-bnd>` 요소에서 `<security-role>` 요소를 작성하십시오. 각 `<security-role>` 요소는 각 역할(**mfpadmin**, mfpdeployer, mfpmonitor, mfpoperator)에 대한 것입니다. 이러한 역할을 적절한 사용자 그룹 이름(이 예에서는 **mfpadmingroup**, **mfpdeployergroup**, **mfpmonitorgroup** 또는 **mfpoperatorgroup**)에 맵핑하십시오. 이러한 그룹은 `<basicRegistry>` 요소를 통해 정의됩니다. 이 요소를 사용자 정의하거나 전체를 `<ldapRegistry>` 요소 또는 `<safRegistry>` 요소로 대체할 수 있습니다.
...
| 특성 | 설명 |
|----------|-------------|
| mfp.scheduler.startHour | 선택사항. iFix 레벨 *8.0.0.0-MFPF-IF201907091643*에서 도입되었습니다. 기본값 오전 1시가 아닌 고객이 선택한 시간으로 스케줄러 실행을 설정할 수 있습니다. 이 특성은 1에서 23 사이의 값을 가질 수 있습니다. 이 특성은 고객이 트래픽이 적은 시간에 스케줄러가 시작하도록 구성할 수 있게 하며, 매일 서버가 시작되더라도 스케줄러가 실행되도록 할 수 있습니다. <br/>**중요 참고**: 구성된 값은 서버 시작 시간과 4시간 차이가 있어야 합니다. 스케줄러 시작 시간이 서버 시작 후 4시간 이내이면 스케줄러가 실행을 건너뛰고 다음 날 같은 시간에 실행하도록 스케줄합니다. |
...
1. **리소스 → JDBC → 데이터 소스 → 데이터 소스 이름**으로 이동하십시오.
...
2. **리소스 → JDBC 제공자 → database_jdbc_provider → 데이터 소스 → your_data_source → 연결 풀 특성**을 선택하십시오.
