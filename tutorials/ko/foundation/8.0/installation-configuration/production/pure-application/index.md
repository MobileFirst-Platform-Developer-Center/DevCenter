...
| 노드 | 컴포넌트 | 
|------|------------|
| MobileFirst Platform Server | **Liberty 프로파일 서버**<br/>WebSphere Application Server Liberty 프로파일 서버 설치입니다.<br/><br/>**MFP Server Prerequisite**<br/>SSL 및 Ant를 포함하는 {{ site.data.keys.mf_server }} 설치의 전제조건입니다.<br/><br/>**MFP Server Administration**<br/>{{ site.data.keys.mf_server }} 관리 웹 애플리케이션({{ site.data.keys.mf_console }} 포함)입니다.<br/>**MFP Server Runtime Deployment**<br/>런타임 컨텍스트 루트 구성입니다.<br/><br/>**MFP Server Application**<br/> 배치에 추가되는 {{ site.data.keys.product_adj }} 애플리케이션입니다.<br/><br/>**MFP Server Adapter**<br/>입니다. 배치에 추가되는 어댑터입니다.<br/><br/>**MFP Server Application Adapter Deployment**<br/>{{ site.data.keys.mf_server }}에 대한 애플리케이션 및 어댑터 배치입니다. | 
| MobileFirst Platform DB | **데이터베이스 서버**<br/>DB2 xdatabase 서버 설치입니다.<br/><br/>**MFP Administration DB**<br/>MobileFirst 관리 데이터베이스 스키마 설치입니다.<br/><br/>**MFP Runtime DB**<br/>{{ site.data.keys.product }} 런타임 데이터베이스 스키마 설치입니다.<br/><br/>**기본값 추가 디스크**<br/>디스크 크기 구성입니다. | 
...
| 노드 | 컴포넌트 | 
|------|------------|
| IHS Server | **IBM  HTTP Server**<br/>IBM HTTP Server 설치입니다.<br/><br/>**MFP IHS 구성**<br/>IBM HTTP Server의 자동 구성입니다. | 
| MobileFirst Platform Server | **Liberty 프로파일 서버**<br/>WebSphere Application Server Liberty 프로파일 서버 설치입니다.<br/><br/>**MFP Server Prerequisite**<br/>SSL 및 Ant를 포함하는 {{ site.data.keys.mf_server }} 설치의 전제조건입니다.<br/><br/>**MFP Server Administration**<br/>{{ site.data.keys.mf_server }} 관리 웹 애플리케이션({{ site.data.keys.mf_console }} 포함)입니다.<br/><br/>**MFP Server Runtime Deployment**<br/>런타임 컨텍스트 루트 구성입니다.<br/><br/>**MFP Server Application**<br/>배치에 추가되는 {{ site.data.keys.product_adj }} 애플리케이션입니다.<br/><br/>**MFP Server Adapter**<br/>배치에 추가되는 어댑터입니다.<br/><br/>**MFP Server Application Adapter Deployment**<br/>{{ site.data.keys.mf_server }}에 대한 애플리케이션 및 어댑터 배치입니다.<br/><br/>**기본 스케일링 정책**<br/>VM 스케일링 정책: VM 수입니다. | 
| MobileFirst Platform DB | **데이터베이스 서버**<br/>DB2 데이터베이스 서버 설치입니다.<br/><br/>**MFP Administration DB**<br/>{{ site.data.keys.product_adj }} 관리 데이터베이스 스키마 설치입니다.<br/><br/>**MFP Runtime DB**<br/>{{ site.data.keys.product }} 런타임 데이터베이스 스키마 설치입니다.<br/><br/>**기본 추가 디스크**<br/>디스크 크기 구성입니다. | 
...
| 노드 | 컴포넌트 | 
|------|------------|
| MobileFirst Platform Server | **독립형 서버**<br/>WebSphere Application Server 전체 프로파일 서버 설치입니다.<br/><br/>제한사항:<br/>다음 컴포넌트 속성의 값을 변경하지 마십시오. {::nomarkdown}<ul><li>셀 이름</li><li>노드 이름</li><li>프로파일 이름</li></ul>{:/}이러한 속성을 변경하는 경우 이 템플리트를 기반으로 하는 패턴의 배치에 실패합니다.<br/><br/>**MFP Server Prerequisite**<br/>SSL 및 Ant를 포함하는 {{ site.data.keys.mf_server }} 설치의 전제조건입니다.<br/><br/>**MFP Server Administration**<br/>{{ site.data.keys.mf_server }} 관리 웹 애플리케이션({{ site.data.keys.mf_console }} 포함)입니다.<br/><br/>**MFP Server Runtime Deployment**<br/>런타임 컨텍스트 루트 구성입니다.<br/><br/>**{{ site.data.keys.product_adj }} 앱**<br/>배치에 추가되는 {{ site.data.keys.product_adj }} 애플리케이션입니다.<br/><br/>**{{ site.data.keys.product_adj }} 어댑터**<br/> 배치에 추가되는 {{ site.data.keys.product_adj }} 어댑터입니다.<br/><br/>**MFP Server Application Adapter Deployment**<br/>{{ site.data.keys.mf_server }}에 대한 애플리케이션 및 어댑터 배치입니다. | 
| MobileFirst Platform DB | **데이터베이스 서버**<br/>DB2 데이터베이스 서버 설치입니다.<br/><br/>**MFP Administration DB**<br/>{{ site.data.keys.product_adj }} 관리 데이터베이스 스키마 설치입니다.<br/><br/>**MFP Runtime DB**<br/>{{ site.data.keys.product }} 런타임 데이터베이스 스키마 설치입니다.<br/><br/>**기본 추가 디스크**<br/>디스크 크기 구성입니다. | 
...
|------|------------|
| IHS Server | **IBM HTTP Server**<br/>IBM HTTP Server 설치입니다.<br/><br/>**MFP IHS 구성**<br/>IBM HTTP Server의 자동 구성입니다. | 
| MobileFirst Platform Server | **독립형 서버**<br/>WebSphere Application Server 전체 프로파일 서버 설치입니다.<br/><br/>제한사항: 다음 컴포넌트 속성의 값을 변경하지 마십시오.{::nomarkdown}<ul><li>셀 이름</li><li>노드 이름</li><li>프로파일 이름</li></ul>{:/}이러한 속성을 변경하는 경우 이 템플리트를 기반으로 하는 패턴의 배치에 실패합니다.<br/><br/>**MFP Server Prerequisite**<br/>SSL 및 Ant를 포함하는 {{ site.data.keys.mf_server }} 설치의 전제조건입니다.<br/><br/>**MFP Server Administration**<br/>{{ site.data.keys.mf_server }} 관리 웹 애플리케이션({{ site.data.keys.mf_console }} 포함)입니다.<br/><br/>**MFP Server Runtime Deployment**<br/>런타임 컨텍스트 루트 구성입니다.<br/><br/>**{{ site.data.keys.product_adj }} 앱**<br/>배치에 추가되는 {{ site.data.keys.product_adj }} 애플리케이션입니다.<br/><br/>**{{ site.data.keys.product_adj }} 어댑터**배치에 추가되는 어댑터입니다.<br/><br/>**MFP Server Application Adapter Deployment**<br/>{{ site.data.keys.mf_server }}에 대한 애플리케이션 및 어댑터 배치입니다.<br/><br/>**기본 스케일링 정책**<br/>VM 스케일링 정책: VM수입니다. | 
| MobileFirst Platform DB | **데이터베이스 서버**<br/>DB2 데이터베이스 서버 설치입니다.<br/><br/>**MFP Administration DB**<br/>{{ site.data.keys.product_adj }} 관리 데이터베이스 스키마 설치입니다.<br/><br/>**MFP Runtime DB**<br/>{{ site.data.keys.product }} 런타임 데이터베이스 스키마 설치입니다.<br/><br/>**기본 추가 디스크**<br/>디스크 크기 구성입니다. | 
...
|------|------------|
| IHS Server | **IBM HTTP Server**<br/>IBM HTTP Server 설치입니다.<br/><br/>**MFP IHS 구성**<br/>IBM HTTP Server의 자동 구성입니다. | 
| DmgrNode | **배치 관리자**<br/>WebSphere Application Server 배치 관리자 설치입니다.<br/><br/>제한사항: 다음 컴포넌트 속성의 값을 변경하지 마십시오.{::nomarkdown}<ul><li>셀 이름</li><li>노드 이름</li><li>프로파일 이름</li></ul>{:/}이러한 속성을 변경하는 경우 이 템플리트를 기반으로 하는 패턴의 배치에 실패합니다.<br/><br/>**MFP Server Prerequisite**<br/>SSL 및 Ant를 포함하는 {{ site.data.keys.mf_server }} 설치의 전제조건입니다.<br/><br/>**MFP Server Administration**<br/>{{ site.data.keys.mf_server }} 관리 웹 애플리케이션({{ site.data.keys.mf_console }} 포함)입니다.<br/><br/>**MFP Runtime**<br/>런타임 WAR 파일입니다.<br/><br/>**MFP Server Runtime Deployment**<br/>런타임 컨텍스트 루트 구성입니다.<br/><br/>**MFP Application**<br/>배치에 추가되는 {{ site.data.keys.product_adj }} 애플리케이션입니다.<br/><br/>**MFP Adapter**<br/>배치에 추가되는 어댑터입니다.<br/><br/>**MFP Server Application Adapter Deployment**<br/>{{ site.data.keys.mf_server }}에 대한 애플리케이션 및 어댑터 배치입니다. | 
| MobileFirst Platform DB | **데이터베이스 서버**<br/>DB2 데이터베이스 서버 설치입니다.<br/><br/>**MFP Administration DB**<br/>{{ site.data.keys.product_adj }} 관리 데이터베이스 스키마 설치입니다.<br/><br/>**MFP Runtime DB**<br/>{{ site.data.keys.product }} 런타임 데이터베이스 스키마 설치입니다.<br/><br/>**기본 추가 디스크**<br/>디스크 크기 구성입니다. | 
| CustomNode | **사용자 정의 노드**<br/>WebSphere Application Server Network Deployment 서버 클러스터에서 셀 및 노드의 세부사항입니다.<br/><br/>제한사항: 다음 컴포넌트 속성의 값을 변경하지 마십시오.{::nomarkdown}<ul><li>셀 이름</li><li>노드 이름</li><li>프로파일 이름</li></ul>{:/}이러한 속성을 변경하는 경우 이 템플리트를 기반으로 하는 패턴의 배치에 실패합니다.<br/><br/>**MFP Open Firewall Ports for WAS**<br/>데이터베이스 서버 및 LDAP 서버에 대한 연결을 사용으로 설정하기 위해 열어야 하는 포트입니다.<br/><br/>**기본 스케일링 정책**<br/>선택한 토폴로지에 필요한 가상 머신 인스턴스의 수입니다. | 
...
| 노드 | 컴포넌트 |
|------|------------|
| MFP AppCenter DB | **데이터베이스 서버**<br/>DB2 데이터베이스 서버 설치입니다.<br/><br/>**기본 추가 디스크**<br/>디스크 크기 구성입니다. | 
| MFP AppCenter Server | **Liberty 프로파일 서버**<br/>WebSphere Application Server Liberty 프로파일 서버 설치입니다.<br/><br/>**MFP Server Prerequisite**<br/>SSL 및 Ant를 포함하는 {{ site.data.keys.mf_server }} 설치의 전제조건입니다.<br/><br/>**MFP Server Application Center**<br/>이 스크립트 패키지는 WebSphere Application Server 전제 프로파일 또는 WebSphere Application Server Liberty 프로파일 서버에서 {{ site.data.keys.mf_app_center }} 서버를 설정합니다. | 
...
|------|------------|
| MFP AppCenter DB | **데이터베이스 서버**<br/>DB2 데이터베이스 서버 설치입니다.<br/><br/>**기본 추가 디스크**<br/>디스크 크기 구성입니다. | 
| MFP AppCenter Server | **독립형 서버**<br/>WebSphere Application Server 전체 프로파일 서버 설치입니다.<br/><br/>제한사항: 다음 컴포넌트 속성의 값을 변경하지 마십시오.{::nomarkdown}<ul><li>셀 이름</li><li>노드 이름</li><li>프로파일 이름</li></ul>{:/}이러한 속성을 변경하는 경우 이 템플리트를 기반으로 하는 패턴의 배치에 실패합니다.<br/><br/>**MFP WAS SDK 레벨**<br/>이 스크립트의 목적은 필요한 SDK 레벨을 WAS 프로파일의 기본 SDK로 설정하는 것입니다.<br/><br/>**MFP Server Prerequisite**<br/>SSL 및 Ant를 포함하는 {{ site.data.keys.mf_server }} 설치의 전제조건입니다.<br/><br/>**MFP Server Application Center**<br/>이 스크립트 패키지는 WebSphere Application Server 전제 프로파일 또는 WebSphere Application Server Liberty 프로파일 서버에서 {{ site.data.keys.mf_app_center }} 서버를 설정합니다. | 
...
| HEAP\_MIN\_SIZE | WebSphere Application Server 전체 프로파일 전용입니다.<br/><br/>생성되는 Analytics 데이터의 양에 따라 추가 데이터 처리를 위해 추가 메모리가 필요합니다. WebSphere Application Server 전체 프로파일의 최소 힙 크기를 증가시킬 수 있도록 이 값을 설정하십시오. {{ site.data.keys.mf_analytics }}의 코어 OS 컴포넌트에서 지정된 메모리 크기가 이 값보다 커야 합니다. HEAP_MAX_SIZE와 동일한 값을 설정하는 것이 좋습니다.<br/><br/>기본값: 4096(MB). | 
| HEAP\_MAX\_SIZE	| WebSphere Application Server 전체 프로파일 전용입니다.<br/><br/>생성되는 Analytics 데이터의 양에 따라 추가 데이터 처리를 위해 추가 메모리가 필요합니다. WebSphere Application Server 전체 프로파일의 최대 힙 크기를 증가시킬 수 있도록 이 값을 설정하십시오. {{ site.data.keys.mf_analytics }}의 코어 OS 컴포넌트에서 지정된 메모리 크기가 이 값보다 커야 합니다. HEAP_MIN_SIZE와 동일한 값을 설정하는 것이 좋습니다.<br/><br/>기본값: 4096(MB). | 
| WAS\_admin\_user | WebSphere Application Server 전체 프로파일 전용입니다.<br/><br/>Analytics 서버용 WebSphere Application Server 전체 프로파일 관리 사용자입니다. WebSphere Application Server Liberty 프로파일의 경우 기본값을 변경하지 않고 그대로 두십시오. | 
| WAS\_admin\_password | WebSphere Application Server 전체 프로파일 전용입니다.<br/><br/>Analytics 서버용 WebSphere Application Server 전체 프로파일 관리 사용자 비밀번호입니다. WebSphere Application Server Liberty 프로파일의 경우 기본값을 변경하지 않고 그대로 두십시오. | 
...
| LDAP_TYPE | (LDAP 매개변수) 필수. 사용자 레지스트리의 LDAP 서버 유형:<br/><br/>없음<br/>LDAP 연결이 사용 불가능합니다. 이 값이 설정되면 모든 기타 LDAP 매개변수가 플레이스홀더 전용으로 처리됩니다.<br/><br/>TivoliDirectoryServer<br/>LDAP 저장소가 IBM Tivoli Directory Server인 경우 이를 선택하십시오.<br/><br/>ActiveDirectory<br/>LDAP 저장소가 Microsoft Active Directory인 경우 이를 선택하십시오.<br/><br/>기본값: 없음. | 
...
| mfpadmin | (LDAP 매개변수) {{ site.data.keys.mf_server }}의 관리자 역할:<br/><br/>없음<br/>사용자가 없습니다.<br/><br/>AllAuthenticatedUsers<br/>인증된 사용자<br/><br/>모두<br/>모든 사용자입니다.<br/><br/>기본값: 없음. | 
| mfpdeployer | (LDAP 매개변수) {{ site.data.keys.mf_server }}의 배치자 역할:<br/><br/>없음<br/>사용자가 없습니다.<br/><br/>AllAuthenticatedUsers<br/>인증된 사용자<br/><br/>모두<br/>모든 사용자입니다.<br/><br/>기본값: 없음. | 
| mfpmonitor | (LDAP 매개변수) {{ site.data.keys.mf_server }}의 모니터 역할:<br/><br/>없음<br/>사용자가 없습니다.<br/><br/>AllAuthenticatedUsers<br/>인증된 사용자<br/><br/>모두<br/>모든 사용자입니다.<br/><br/>기본값: 없음. | 
| mfpoperator | (LDAP 매개변수) {{ site.data.keys.mf_server }}의 운영자 역할:<br/><br/>없음<br/>사용자가 없습니다.<br/><br/>AllAuthenticatedUsers<br/>인증된 사용자<br/><br/>모두<br/>모든 사용자입니다.<br/><br/>기본값: 없음. | 
...
| profile_name |선택사항. WebSphere Application Server 런타임 환경의 파일을 포함하는 프로파일 이름입니다.<br/><br/>패턴 템플리트에서 이는 출력 속성에 맵핑됩니다. 배치 관리자의 **dmgr\_profile\_name** 또는 독립형 서버의 sa_profile_name입니다. | 
| runtime\_contextRoot\_list | 필수. IHS가 일치하는 컨텍스트 루트가 있는 요청을 라우팅하는 데 사용되는 런타임 컨텍스트 루트 목록입니다. 세미콜론(;)을 사용하여 런타임 컨텍스트 루트를 구분하십시오. 예를 들어, HelloMobileFirst;HelloWorld입니다.<br/><br/>중요: MFP Server Runtime Deployment에서 지정된 컨텍스트 루트와 일치해야 합니다. 그렇지 않으면 IHS가 런타임 컨텍스트 루트가 포함된 요청을 올바르게 라우팅할 수 없습니다. | 
...
| LDAP\_REPOSITORY\_NAME | (LDAP 매개변수) LDAP 서버 이름입니다. | 
| CERT\_FILE\_PATH | (LDAP 매개변수) 업로드된 LDAP 서버 인증의 대상 경로입니다. REQUIRE_SSL이 true로 설정된 경우 필수입니다. | 
| mfpadmin | {{ site.data.keys.mf_server }}의 관리자 역할입니다. 다음 값 중 하나입니다.<br/><br/>없음<br/>사용자가 없습니다.<br/><br/>AllAuthenticatedUsers<br/>인증된 사용자<br/><br/>모두<br/>모든 사용자입니다.<br/><br/>기본값: 없음. | 
| mfpdeployer | (LDAP 매개변수) {{ site.data.keys.mf_server }}의 배치자 역할:<br/><br/>없음<br/>사용자가 없습니다.<br/><br/>AllAuthenticatedUsers<br/>인증된 사용자<br/><br/>모두<br/>모든 사용자입니다.<br/><br/>기본값: 없음. | 
| mfpmonitor | (LDAP 매개변수) {{ site.data.keys.mf_server }}의 모니터 역할:<br/><br/>없음<br/>사용자가 없습니다.<br/><br/>AllAuthenticatedUsers<br/>인증된 사용자<br/><br/>모두<br/>모든 사용자입니다.<br/><br/>기본값: 없음. | 
| mfpoperator | (LDAP 매개변수) {{ site.data.keys.mf_server }}의 운영자 역할:<br/><br/>없음<br/>사용자가 없습니다.<br/><br/>AllAuthenticatedUsers<br/>인증된 사용자<br/><br/>모두<br/>모든 사용자입니다.<br/><br/>기본값: 없음. | 
...
| db_ip | 관리 데이터베이스가 설치되는 DB 서버의 IP 주소입니다. 패턴 템플리트에서 Database Server 소프트웨어 컴포넌트의 IP 출력 속성에 맵핑됩니다. | 
| db_port | 관리 데이터베이스가 설치되는 DB 서버의 포트 번호입니다. 패턴 템플리트에서 Database Server 소프트웨어 컴포넌트의 instancePort 출력 속성에 맵핑됩니다.|
| admin_user | {{ site.data.keys.mf_server }} 관리 권한이 있는 사용자 이름입니다.<br/><br/>패턴 템플리트에서 이는 패턴 레벨 매개변수로서 MFP Server Administration 스크립트 패키지에 있는 동일한 이름의 매개변수와 연관되어 동일한 값으로 설정됩니다. | 
| admin_password | 관리 사용자 비밀번호입니다.<br/><br/>패턴 템플리트에서 이는 패턴 레벨 매개변수로서 MFP Server Administration 스크립트 패키지에 있는 동일한 이름의 매개변수와 연관되어 동일한 값으로 설정됩니다. | 
| WAS\_admin\_user | WebSphere Application Server의 경우 필수입니다. WebSphere Application Server Liberty의 경우 선택사항입니다. {{ site.data.keys.mf_server }} 배치가 WebSphere Application Server 전체 프로파일에서 완료되면 패턴 템플리트에서 독립형 서버의 was_adminoutput 속성에 맵핑됩니다.<br/><br/>{{ site.data.keys.mf_server }} 배치가 WebSphere Application Server Network Deployment에서 완료되면 패턴 템플리트에서 배치 관리자의 was_admin 출력 속성에 맵핑됩니다. | 
...
| LDAP\_REPOSITORY\_NAME | (LDAP 매개변수) LDAP 서버 이름입니다. | 
| CERT\_FILE\_PATH | (LDAP 매개변수) 업로드된 LDAP 서버 인증의 대상 경로입니다. REQUIRE_SSL이 true로 설정된 경우 필수입니다. | 
| appcenteradmin | {{ site.data.keys.mf_app_center }}의 관리자 역할입니다. 다음 값 중 하나를 사용하십시오.<ul><li>없음</li><li>사용자 없음</li><li>AllAuthenticatedUsers</li>인증된 사용자</li><li>Everyone</li><li>모든 사용자</li></ul>{:/}기본값: None | 
