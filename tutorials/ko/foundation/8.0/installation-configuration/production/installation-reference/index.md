...
단일 데이터베이스를 선언해야 합니다. `<database kind="ApplicationCenter">`. `<database>` 요소에는 다음 예외를 제외하고 `<configuredatabase>` Ant 태스크와 비슷하게 저정합니다. 예외: `<database>` 요소에는 `<dba>` 및 `<client>` 요소가 없습니다. `<property>` 요소가 있을 수 있습니다.
