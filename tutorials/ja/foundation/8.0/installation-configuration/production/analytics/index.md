...
analytics WAR ファイルおよび EAR ファイルは、MobileFirst Server インストールと共に格納されます。 詳しくは、「MobileFirst Server の配布構造」を参照してください。 WAR ファイルをデプロイする場合、`http://<hostname>:<port>/analytics/console` で MobileFirst Analytics Console を使用できます (例えば、`http://localhost:9080/analytics/console`)。
