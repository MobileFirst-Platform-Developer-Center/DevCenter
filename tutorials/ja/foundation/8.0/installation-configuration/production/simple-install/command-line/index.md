...
* XML ファイルのコピーを編集し、以下のプロパティーの値を設定してください。
    ...
    * **mfp.analytics.receiver.configure** を **false** に。 {{ site.data.keys.mf_analytics_receiver }} への接続は、このチュートリアルの対象範囲に含まれていません。 その他のプロパティー mfp.analytics.receiver.**** は無視して構いません。
