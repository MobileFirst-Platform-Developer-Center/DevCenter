...
* OpenShift 4.2 Cluster の Elasticsearch ノード・ディスカバリーの問題により、デプロイメント内の Mobile Foundation Analytics ポッドの機能に不整合が見られる場合があります。 その結果、Analytics 管理コンソールは、どの時点でも 1 つのノードのみをリストします。
* AppCenter Android モバイル・クライアントのアプリでは、ユーザーがスクロールの問題に気付く場合があります。 これは機能の中断ではありません。 すべてが期待どおりに動作しますが、下にスクロールするにはさらにスクロールが必要です。 これは、AppCenter クライアントで使用されるサード・パーティーの Javascript フレームワークの制限です。
...
### Android アプリケーション・バンドル・フィーチャー

Android 5.x と Android 6.x 以降のデバイス間のアプリケーション・バンドルのパッケージ構造の違いにより、アプリケーション認証性が有効にされ、Play Store 上にアプリケーション・バンドルとして配布されている場合、Android 5.x のユーザーは MobileFirst Server に接続できません。 この制限は、Android 5.x、アプリケーション認証性、および Android アプリケーション・バンドルが組み合わさった場合にのみ存在します。 回避策としては、アプリを通常の .apk ファイルとして Play Store にアップロードするか、Android の最小バージョンを Android 6 以上に制限するか、Android 5.x デバイス用に別のバージョンのアプリを用意するかします。
