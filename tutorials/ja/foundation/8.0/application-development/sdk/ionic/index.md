...
`mfpdev app register` CLI コマンドは、まず、{{ site.data.keys.mf_server }} に接続してアプリケーションを登録し、次に、Ionic プロジェクトのルートにある **config.xml** ファイルを、{{ site.data.keys.mf_server }} を識別するメタデータで更新します。
