...
安全框架围绕实施 OAuth 协议的授权服务器而构建，并公开客户机与之交互以获取访问令牌的 OAuth 端点。安全框架提供构建块，用于在授权服务器和底层 OAuth 协议上实施定制授权逻辑。缺省情况下，{{ site.data.keys.mf_server }} 也充当**授权服务器**。但是，您可以配置 IBM WebSphere DataPower 设备以充当授权服务器并与 {{ site.data.keys.mf_server }} 进行交互。
...
缺省情况下，在**作用域**中编写的**作用域元素**将映射到**具有相同名称的安全性检查**。例如，如果编写名为 `PinCodeAttempts` 的安全性检查，那么可在作用域中使用具有相同名称的作用域元素。
