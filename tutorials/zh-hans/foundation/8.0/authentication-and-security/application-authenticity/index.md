...
#### 可用性
{: #availability }
* 应用程序真实性功能在所有受支持平台（iOS、watchOS、Android、Windows 8.1 Universal、Windows 10 UWP）的 Cordova 和本机应用程序中可用。
...
## 构建时密钥 (BTS)
{: #bts }
构建时密钥 (BTS) 是**用于增强真实性验证的可选工具**（仅适用于 iOS 和 watchOS 应用程序）。该工具在应用程序中插入在构建时确定的密钥，稍后用于真实性验证过程。
...
3. 将**运行脚本阶段**拖至**编译源代码**阶段上方。
4. 仅当应用程序环境为 watchOS 时才需要执行此步骤。要启用 BTS，开发人员必须传递打包在 watchOS 扩展中的 swift 文件名（在 BTS 工具位置信息之后传递）。

例如，假设 watchOS 扩展包包含 `HelloWatchOS.swift` 文件，那么开发人员必须将 `HelloWatchOS` 作为自变量与 BTS 工具的路径一起传递。
