...
If you configured HTTPS protocol in previous step for `mfp-server-url`, you must configure API Connect TLS Profile in the **proxy** policy component.  
Go to the **proxy**  policy component in the **Assemble** tab and choose in **TLS Profile** property the one you created in previous step of **"Add API Connect TLS Profile for MobileFirst HTTPS endpoint"**.

![Choose TLS Profile](tls-proxy-policy.png)
...
4. To determine full URL path of MobileFirst OAuthProvider for /oauth2/authorize, concatenate the `Base URL` of **MobileFirst OAuthProvider** value (`/mfpProvider`) and the `/oauth2/authorize` endpoint.

 The full URL of the MobileFirst OAuthProvider should look like this:

`https://{DataPowerGateway}/{organizationName}/{catalogName}/mfpProvider/oauth2/authorize`

### Create Simple REST API in API Connect
In this example we create an API called **Branches** and a Product called **Bank** (Based on API Connect tutorial of [Creating an invoke REST API definition](http://www.ibm.com/support/knowledgecenter/SSMNED_5.0.0/com.ibm.apic.toolkit.doc/tutorial_apionprem_apiproxy.html)).
...
7. Go to the **Assemble** tab and click on the **invoke** policy component that appears in the **Assemble** pane.
...
In API Connect protecting an API is done by Security Definition.

1. In the newly created Branches API, choose **Security Definition** from the design list click the <img alt="save button" style="margin:0;display:inline" src="addIcon.png"/> button to add a defintion of type **OAuth**.
...
3. Go to the **Paths** section and choose `/details` path.
...
1. To create a Prodcut definition, click the <img alt="save button" style="margin:0;display:inline" src="addIcon.png"/> add button and then select **New Product from scratch**.

2. Configure the following parameters and click on 'Create product' button to create the API:
...
3. The **Design** tab of the Prodcut will be displayed.

4. Go to **APIs** section click **Add** <img alt="save button" style="margin:0;display:inline" src="addIcon.png"/> .

    ![Prodcut add APIs](product_add_apis.png)

5. Add the two APIs(which you created before): **MobileFirst OAuthProvider** and **Branches**.
...
See [Configuring SSL by using untrusted certificates](http://www.ibm.com/support/knowledgecenter/SSHSCD_7.1.0/com.ibm.worklight.installconfig.doc/admin/c_ssl_config.html) for more details how to add untrusted certificate to your client application.  

For Android clients (like **PinCodeAndroid** sample), you can follow [Using custom certificates and hosts file with Android emulator Blog](https://www.ibm.com/blogs/sweeden/using-custom-certificates-and-hosts-file-with-android-emulator) or
