...
* Using Non-MobileFirst Foundation APIs to interact with that same secured server may lead to unexpected behavior, such as the mobile operating system caching a non-verified certificate. Make sure to only use MobileFirst Foundation APIs for all communications with that hostname, even after pinning.
