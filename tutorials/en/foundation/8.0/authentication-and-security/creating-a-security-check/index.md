...
> <b>Note:</b> While security checks are implemented within adapters, the {{ site.data.keys.mf_product_adj }} security-framework and adapter APIs are separate and cannot be mixed. Therefore, you cannot use an adapter API, such as the `Adpaters.API` interface, in your security-check code, and you cannot use security-check APIs in adapter resource code.
