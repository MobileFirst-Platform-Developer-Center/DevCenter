...
## Présentation
{: #overview }
Vous pouvez installer des applications sur votre terminal mobile avec le client mobile Application Center.

Le client mobile Application Center est l'application qui s'exécute sur votre terminal Android, iOS, Windows Phone ou Windows. Seul Windows Phone 8 est pris en charge par la version actuelle d'Application Center. Vous utilisez le client mobile pour répertorier le catalogue d'applications disponibles dans Application Center. Vous pouvez installer ces applications sur votre terminal. Le client mobile est parfois appelé programme d'installation d'Application Center. Cette application doit être présente sur votre terminal si vous souhaitez installer sur votre terminal des applications à partir de votre référentiel d'applications privé.

### Prérequis
{: #prerequisites }
Votre administrateur système doit vous fournir un nom d'utilisateur et un mot de passe pour que vous puissiez télécharger et installer le client mobile. Le nom d'utilisateur et le mot de passe sont requis chaque fois que vous démarrez le client mobile sur votre terminal. Pour les applications Windows Store, le nom d'utilisateur et le mot de passe sont requis pour le client mobile uniquement au moment de l'exécution. Pour des raisons de sécurité, ne diffusez pas ces données d'identification. Ces données d'identification sont identiques à celles utilisées pour se connecter à la console Application Center.

#### Accéder à
{: #jump-to }
* [Installation du client sur un terminal mobile Android](#installing-the-client-on-an-android-mobile-device)
* [Installation du client sur un terminal mobile iOS](#installing-the-client-on-an-ios-mobile-device)
* [Installation du client sur un terminal mobile Windows Phone 8 Universal](#installing-the-client-on-a-windows-phone-8-universal-mobile-device)
* [Vue de connexion](#the-login-view)
* [Vues dans le client Application Center](#views-in-the-application-center-client)
* [Installation d'une application sur un terminal Android](#installing-an-application-on-an-android-device)
* [Installation d'une application sur un terminal iOS](#installing-an-application-on-an-ios-device)
* [Installation d'une application sur un terminal Windows Phone](#installing-an-application-on-a-windows-phone-device)
* [Installation d'une application Windows Store sur un terminal Windows](#installing-a-windows-store-application-on-a-windows-device)
...
## Installation du client sur un terminal mobile Android
{: #installing-the-client-on-an-android-mobile-device }
Vous pouvez installer le client mobile ou toute application signée portant l'indicateur d'installation sur votre terminal mobile Android en saisissant l'URL d'accès dans votre navigateur, en saisissant vos données d'identification et en effectuant les étapes requises.

1. Démarrez le navigateur sur votre terminal mobile.
2. Entrez l'URL d'accès suivante dans la zone de texte de l'adresse : `http://hostname:portnumber/applicationcenter/installers.html`
    ...
    Application Center fournit également une autre URL pour installer le client sur un terminal mobile : `http://hostname:portnumber/applicationcenter/inst.html`. La page de cette URL fonctionne mieux avec certains navigateurs Web mobiles plus anciens ou non standard. Si la page installers.html ne fonctionne pas sur votre terminal mobile, vous pouvez utiliser inst.html. Cette page est fournie en anglais seulement et n'est pas traduite.

    Si vous tentez d'ouvrir la page avec HTTPS et d'utiliser des certificats auto-signés, les anciens navigateurs Android ne peuvent pas ouvrir cette page. Dans ce cas, vous devez utiliser un certificat non auto-signé ou utiliser un autre navigateur sur le terminal Android, tel que Firefox, Chrome ou Opera. Dans Android 4 et versions ultérieures, le navigateur Android affiche un avertissement de sécurité concernant le certificat SSL, mais vous permet d'accéder au site Web après confirmation que vous consentez à une connexion non sécurisée.

3. Entrez votre nom d'utilisateur et votre mot de passe. Lorsque votre nom d'utilisateur et votre mot de passe sont validés, la liste des applications d'installation compatibles de votre terminal s'affiche dans le navigateur. Normalement, une seule application, le client mobile, apparaît dans cette liste.

4. Si le serveur Web utilise un certificat d'AC auto-signé, installez le certificat au moins une fois sur le terminal. L'administrateur Application Center doit fournir le certificat ; voir [Gestion et installation de certificats d'AC auto-signés dans un environnement de test Application Center](../../installation-configuration/production/appcenter/#managing-and-installing-self-signed-ca-certificates-in-an-application-center-test-environment) pour plus de détails.
    * Appuyez sur l'onglet **SSL-Certificate** et sélectionnez le certificat.
    * Appuyez sur **Install**. Vous devez effectuer cette opération une seule fois pour le terminal. Vous pouvez vérifier si le certificat est installé en accédant à **Settings → Security → Trusted Credentials → User on the device**. Cette vue affiche les certificats SSL que l'utilisateur a installés sur le terminal. Si le certificat d'AC auto-signé n'est pas installé sur le terminal, le système d'exploitation Android vous empêche de télécharger le client mobile dans les étapes suivantes.
    ...
    ![Liste des applications de client mobile à installer](ac_instal_client_and1.jpg)
...
6. Appuyez sur **Install Now** pour télécharger le client mobile. Sur les nouveaux terminaux Android, le système peut demander à Chrome l'accès aux fichiers de support sur le terminal. Répondez oui. Un avertissement concernant les fichiers potentiellement dangereux peut être affiché. Sélectionnez l'option permettant de conserver le fichier APK.
...
* Le terminal ne dispose pas de suffisamment de mémoire.
* Le certificat SSL du serveur n'est pas connu du terminal.

La première fois que vous installez une application via Downloads, vous pouvez recevoir une demande pour confirmer si Google doit vérifier régulièrement l'activité du terminal à des fins de sécurité. Vous pouvez accepter ou refuser Le client Application Center n'est pas affecté par votre choix.

L'installation peut être bloquée pour l'une des raisons suivantes :

* Le terminal ne permet pas l'installation à partir de sources inconnues. Accédez à **Settings → Security** sur le terminal et activez **Unknown sources (Allow installation from unknown sources)**.
* La même application est déjà installée sur le terminal, mais elle a été signée par un autre certificat. Dans ce cas, vous devez supprimer l'application avant de l'installer sur le terminal avec un autre certificat signé.

## Installation du client sur un terminal mobile iOS
{: #installing-the-client-on-an-ios-mobile-device }
Vous pouvez installer le client mobile ou toute application signée portant l'indicateur d'installation sur votre terminal mobile iOS en saisissant l'URL d'accès dans votre navigateur, en saisissant vos données d'identification et en effectuant les étapes requises.

> **Important :** pour installer des applications sur des terminaux iOS, vous devez d'abord configurer le serveur Application Center avec SSL. Voir [Configuration de Secure Sockets Layer (SSL)](../../installation-configuration/production/appcenter/#configuring-secure-sockets-layer-ssl).

La propriété JNDI **ibm.appcenter.ios.plist.onetimeurl** des services IBM Application Center contrôle si les URL uniques sont utilisées lorsque le client mobile est installé sur un terminal mobile iOS. Définissez cette propriété sur false pour une sécurité maximale. Lorsque vous définissez cette propriété sur false, les utilisateurs doivent entrer leurs données d'identification plusieurs fois lorsqu'ils installent le client mobile : une fois lorsqu'ils sélectionnent le client et une fois lorsqu'ils installent le client.
...
L'installation du client mobile sur un terminal iOS est semblable à une installation sur Android, mais avec quelques différences. Le programme d'installation démarre automatiquement après le téléchargement. Les données d'identification de nom d'utilisateur et de mot de passe sont requises pour presque toutes les étapes d'installation.
...
3. Entrez votre nom d'utilisateur et votre mot de passe. Consultez les [prérequis du client mobile](../preparations).
    ...
    Lorsque votre nom d'utilisateur et votre mot de passe sont validés, la liste des applications d'installation compatibles de votre terminal s'affiche dans le navigateur. Normalement, une seule application, le client mobile, apparaît dans cette liste.
...
4. Si le serveur Web utilise un certificat d'AC auto-signé, installez le certificat au moins une fois sur le terminal.
    ...
    * Appuyez sur l'onglet SSL-Certificate et sélectionnez le certificat.
    * Appuyez sur Install. Vous n'effectuez cette action qu'une seule fois pour le terminal. Vous pouvez vérifier si le certificat est installé en accédant à **Settings → General → Profiles on the device**. Cette vue affiche les certificats SSL que l'utilisateur a installés sur le terminal. Si le certificat d'AC auto-signé n'est pas installé sur le terminal, le système d'exploitation iOS vous empêche de télécharger le client mobile dans les étapes suivantes.
...
> **Remarque :** l'installation d'une application sur un terminal nécessite un profil de mise à disposition qui permet à l'application d'être installée sur le terminal sélectionné. Si vous essayez accidentellement d'installer une application qui n'est pas valide pour votre terminal, certaines versions d'iOS pourraient tenter d'installer l'application dans une boucle sans fin sans jamais réussir ni indiquer d'erreur. L'icône de l'application qui montre la progression de l'installation apparaît sur l'écran d'accueil, mais, en raison de la boucle sans fin, il est difficile de supprimer cette icône d'application pour arrêter la boucle sans fin. Une solution de contournement consiste à mettre le terminal en mode Avion. Dans ce mode, la boucle sans fin est arrêtée et vous pouvez supprimer l'icône de l'application en suivant les étapes normales de suppression d'applications sur des terminaux iOS.

L'installation peut être bloquée pour l'une des raisons suivantes :

* Le profil de mise à disposition de l'application n'est pas valide pour le terminal. L'application doit être signée avec un profil de mise à disposition différent.
* Le terminal n'a aucun accès aux serveurs Apple pour confirmer la validité du profil de mise à disposition.
* Le certificat SSL du serveur n'est pas connu du terminal.

En général, les applications iOS ne peuvent être installées sur le terminal que si elles sont signées avec un profil de mise à disposition. Voir [Importation et génération du projet (Android, iOS, Windows Phone)](../preparations/#importing-and-building-the-project-android-ios-windows-phone).

Depuis iOS 9, lorsqu'une application de société est ouverte, selon le type de profil de mise à disposition, un message Untrusted Enterprise Developer peut s'afficher. Ce message explique que le profil de mise à disposition n'est pas encore approuvé sur ce terminal. Dans ce cas, l'application ne s'ouvre pas, sauf si la confiance est établie pour ce profil de mise à disposition. L'établissement de la confiance doit être effectué une seule fois par profil de mise à disposition.
...
3. Entrez vos données d'identification pour autoriser l'accès au serveur. Sur la partie inférieure de l'écran, une barre d'outils contient un onglet **Installers** et un onglet **Tokens**.
    ...
    ![Préparation à l'installation de jetons et d'applications sur un terminal Windows Phone](ac_wp8_toolbar_inst_app.jpg)
...
4. Appuyez sur **Tokens** et sélectionnez un jeton d'inscription d'application dans la liste des jetons disponibles pour afficher les détails du jeton.
    ...
    ![Détails de jeton d'inscription d'application sur un terminal Windows Phone](ac_wp8_aet_details.jpg)
...
6. Appuyez sur **Add** pour ajouter le compte de la société.
    ...
    ![Ajout d'un compte de société dans un terminal Windows 8 Universal](ac_wp8_add_co_account.jpg)
...
9. Appuyez sur **Install** pour télécharger l'application sélectionnée.
    ...
    ![Application sélectionnée pour téléchargement sur un terminal Windows Phone](ac_wp8_app_details.jpg)
    
10. Appuyez sur **Install** pour installer l'application.

    ![Installation de l'application téléchargée sur un terminal Windows Phone](ac_wp8_app_install.jpg)
    
    Windows 8 Universal ne fournit aucun commentaires en retour sur l'installation de l'application.

> **Astuce :** lorsque vous installez une application de société sur un terminal, ce terminal doit se connecter au serveur Microsoft pour vérifier la signature de l'application. Parfois, le serveur Microsoft ne répond pas. Dans ce cas, vous pouvez recommencer l'installation quelques minutes plus tard. Reportez-vous à la section Installation d'une application sur un terminal Windows Phone pour connaître les messages d'erreur possibles.
...
## Vue de connexion
{: #the-login-view }
Dans la vue de connexion, vous pouvez accéder aux zones requises pour vous connecter au serveur afin d'afficher la liste des applications disponibles pour votre terminal.  
Utilisez la vue de Connexion pour saisir vos données d'identification afin de vous connecter au serveur d'Application Center pour afficher la liste des applications disponibles pour votre terminal.
...
{: #secure-socket-layer-ssl }
Le protocole SSL est obligatoire sur les terminaux iOS. Par conséquent, cette option n'est pas affichée dans la vue de connexion.
...
Utilisez ces vues pour communiquer avec un serveur pour envoyer ou récupérer des informations sur les applications ou pour gérer les applications se trouvant sur votre terminal.
...
* **Catalog**. Cette vue montre les applications qui peuvent être installées sur un terminal.
* **Favorites**. Cette vue affiche la liste des applications que vous avez marquées comme favorites.
* **Updates**. Cette vue affiche toutes les applications que vous avez marquées comme favorites et dont une version ultérieure à celle installée sur le terminal est disponible dans Application Center, le cas échéant.
...
* Dans les applications Android, iOS et Windows Phone, appuyez sur le bouton de déconnexion : <img src="ac_logout_icon.jpg" style="margin:0;display:inline" alt="Bouton pour revenir à la page de connexion et se déconnecter sur les terminaux Android et iOS"/>
...
* Évaluer la version de l'application si elle est installée sur le terminal.
...
## Installation d'une application sur un terminal Android
{: #installing-an-application-on-an-android-device }
Dans la vue **Details**, vous pouvez installer une application sur votre terminal Android.  
Dans la vue Details, si une version précédente de l'application n'est pas installée, vous pouvez installer cette version de l'application sur votre terminal Android.

![Vue détaillée d'une version d'application affichée sur votre terminal Android](ac_phone_app_details.jpg)
...
1. Dans la vue **Details**, appuyez sur **Install**.
    ...
    ![Droits d'accès d'application sur votre terminal Android](ac_app_rights_and.jpg)
...
## Installation d'une application sur un terminal iOS
{: #installing-an-application-on-an-ios-device }
Dans la vue **Details**, vous pouvez installer une version d'application sur votre terminal mobile iOS.

![Vue détaillée d'une version d'application affichée sur votre terminal mobile iOS](ac_phone_app_details_ios.jpg)
...
2. Appuyez sur **Install** pour confirmer le téléchargement et l'installation de la version de l'application ou sur **Cancel** pour annuler l'installation.
    ...
    ![Annulation de l'installation de l'application sur votre terminal iOS](ac_app_inst_cancel_ios.jpg)
    
    Selon l'action effectuée, l'application est installée ou non. Lorsque l'application est installée, elle est également marquée comme application favorite.

    L'installation d'une application sur un terminal nécessite un profil de mise à disposition qui permet à l'application d'être installée sur le terminal sélectionné. Si vous tentez accidentellement d'installer une application qui n'est pas valide pour votre terminal, IOS 6 (obsolète) ou une version antérieure émet un message d'erreur.
...
Certaines versions d'iOS 7 peuvent tenter d'installer l'application dans une boucle sans fin sans jamais réussir ni indiquer d'erreur. L'icône de l'application qui montre la progression de l'installation apparaît sur l'écran d'accueil, mais, en raison de la boucle sans fin, il est difficile de supprimer cette icône d'application pour arrêter la boucle sans fin. Une solution de contournement consiste à mettre le terminal en mode Avion. Dans ce mode, la boucle sans fin est arrêtée et vous pouvez supprimer l'icône de l'application en suivant les étapes normales de suppression d'applications sur des terminaux iOS.

Une fois l'application installée sur le terminal, vous pouvez l'ouvrir.
...
## Installation d'une application sur un terminal Windows Phone
{: #installing-an-application-on-a-windows-phone-device }
Dans la vue **Details**, vous pouvez installer une application de société sur votre terminal Windows Phone.  
La vue **Details** de l'application sélectionnée affiche des informations sur l'application que vous souhaitez installer.

![Vue détaillée d'une version d'une application de société pour l'installation sur un terminal Windows Phone](ac_phone_app_details_wp.jpg)
...
1. Dans la vue **Details**, appuyez sur **Install**. L'application est téléchargée et installée. Vous pouvez appuyer sur **Cancel** à tout moment pendant le téléchargement de l'application pour annuler l'activité. Le bouton **Cancel** apparaît uniquement pendant l'étape de téléchargement du processus d'installation.
    ...
    Au début du processus d'installation, vous êtes invité à confirmer si vous souhaitez ajouter l'application de la société aux applications installées sur votre terminal mobile.
...
2. Appuyez sur **Install** pour confirmer l'installation de l'application ou sur **Cancel** pour annuler l'installation. L'application est marquée comme une application favorite.
    ...
    ![Confirmation ou annulation de l'installation d'une application de société sur un terminal Windows Phone](ac_confirm_instal_app_wp.jpg)
    
> **Astuce :** lorsque vous installez une application de société sur un terminal, ce terminal doit se connecter au serveur Microsoft pour vérifier la signature de l'application. Parfois, le serveur Microsoft ne répond pas. Dans ce cas, vous pouvez recommencer l'installation quelques minutes plus tard.
...
* **Avant d'installer cette application, vous devez ajouter ... compte de société.**
    ...
    Le fichier de module d'application Windows Phone (.xap) est signé, mais le terminal n'est pas inscrit pour les applications de société. Vous devez d'abord installer sur le terminal le jeton d'inscription de l'application de la société.
...
> **Remarque :** si un terminal est enregistré sur plusieurs comptes de société, le système d'exploitation Windows Phone peut afficher un compte de société incorrect dans le message **Would you like to install application from company name?**. Ce message est hors du contrôle d'Application Center. Cette situation est un problème d'affichage uniquement et n'affecte pas la fonctionnalité.
...
> 
Lorsque vous renouvelez votre certificat PFX de signature dans le code, vous devez également renouveler le jeton d'inscription et le déployer sur la console Application Center. Les terminaux doivent également être réinscrits dans le compte de la société avec ce nouveau jeton. Les utilisateurs de terminaux inscrits avec un jeton expiré ne peuvent pas installer d'applications.
...
## Installation d'une application Windows Store sur un terminal Windows
...
L'utilisateur de terminal nécessite des droits d'administrateur sur le terminal pour exécuter le client Application Center.

L'installation des modules APPX via Application Center se fait par un processus appelé chargement indépendant (sideloading). Dans le cadre de la mise à jour de Windows 8.1, le chargement indépendant (sideloading) est activé pour tous les terminaux Windows 8.1 Pro qui font partie d'un domaine Active Directory, ce qui correspond au comportement actuel de Windows 8.1 Enterprise. Si vous utilisez l'une de ces versions de produit et que le terminal fait partie d'un domaine Active Directory, ne vous préoccupez pas des clés de chargement indépendant (sideloading) ou de l'activation de chargement indépendant (sideloading).
...
1. Importez ce certificat dans le magasin "Trusted Root Certification Authorities" de la "machine locale".
    ...
    * Sélectionnez "Local Machine" et cliquez sur **Next**.
    ![Spécification de la machine locale dans l'aAssistant d'importation de certificats](cert_localmachine_win8.jpg)    
    * Sélectionnez "Place all certificate in the following store" puis parcourez pour sélectionner "Trusted Root Certification Authorities".
    ![Placement du certificat dans "Trusted Root Certificate Authorities"](cert_trustedroot_win8.jpg)    
    * Cliquez sur **Next**, puis sur **Finish**. L'importation réussie du certificat doit être confirmée.

Les étapes suivantes décrivent comment effectuer l'installation d'une application Windows Store sur un terminal Windows à l'aide d'Application Center.
...
2. Sélectionnez l'application que vous souhaitez installer pour accéder à ses détails.
    ...
    ![Vue détaillée pour l'installation d'une application Windows Store](ac_details_wstore_app.jpg)
...
Vous pouvez vous connecter à partir du client mobile à des applications stockées dans des magasins d'applications publics pris en charge et installer ces applications sur votre terminal compatible en suivant la procédure normale du magasin d'applications public.

L'administrateur d'Application Center peut créer des liens vers des applications sélectionnées stockées dans des magasins d'applications publics pris en charge et les mettre à la disposition des utilisateurs du client mobile d'Application Center sur les systèmes d'exploitation qui correspondent à ces applications. Voir [Ajout d'une application à partir d'un magasin d'applications public](../appcenter-console/#adding-an-application-from-a-public-app-store). Vous pouvez installer ces applications via le client mobile sur votre terminal compatible.

Les liens vers des applications Android stockées dans Google Play et vers des applications iOS stockées dans Apple iTunes sont répertoriés dans la liste des applications du terminal avec les fichiers binaires des applications privées créées dans votre société.
...
2. Appuyez sur **Go to Store** pour ouvrir Google Play ou Apple iTunes.
    ...
    **Android**
    ![Accès à une application dans Google Play depuis le client mobile sur le terminal](ac_and_link_app.jpg)
    
    **iOS**
    ![Accès à une application dans Apple iTunes depuis le client mobile sur le terminal](ac_ios_link_app.jpg)
...
Vous pouvez supprimer une application installée sur votre terminal mobile.

1. Démarrez la procédure de suppression valide pour le système d'exploitation de votre terminal.
...
2. **Android uniquement** : supprimer une application d'un terminal Android.
...
Vous pouvez afficher les détails de la version sélectionnée d'une application en suivant la procédure appropriée pour un téléphone ou une tablette Android ou iOS, un terminal Windows Phone ou un terminal Windows.

1. Affichez les détails d'une version d'application spécifique sur un terminal mobile en sélectionnant la procédure appropriée pour votre terminal.
    * Téléphone Windows Phone, Android ou iOS : voir l'étape 2.
    * Terminal Windows : voir l'étape 3.
...
2. **Windows Phone, Android, iOS uniquement** : affichez les détails d'une version d'application spécifique sur un téléphone Windows Phone, Android ou iOS.
    * Appuyez sur **Select a version** pour accéder à la vue de la liste des versions.
    ![Version spécifique d'une application sélectionnée dans la liste des versions sur un téléphone Windows Phone, Android ou iOS](ac_version_select.jpg) 
...
3. Windows uniquement : affichez les détails d'une version d'application Windows Store spécifique sur un terminal Windows. Si plusieurs versions sont disponibles pour l'application Windows Store, vous pouvez sélectionner la version que vous souhaitez installer.
    * Appuyez sur la version appropriée de l'application. La vue **Details** est mise à jour et affiche les détails de la version de l'application sélectionnée.
...
3. **Android et Windows 8 Universal** : sur les terminaux Android et Windows 8 Universal, appuyez sur **Update**.
4. **iOS uniquement** : sur les terminaux iOS, appuyez sur **Install latest**.
...
Vous pouvez activer la détection automatique de nouvelles versions de l'application client. Ensuite, vous pouvez choisir de télécharger ou d'installer la nouvelle version sur votre terminal mobile. Cette fonctionnalité est prise en charge pour iOS, Android et Windows Phone.
...
2. Appuyez sur **Install** pour installer la version ultérieure de l'application.
    ...
    ![Confirmez l'installation de la version mise à jour de l'application](ac_client_autoupgrade_install_app.jpg)
...
Suivez ensuite la procédure décrite dans [Installation d'une application sur un terminal Android](#installing-an-application-on-an-android-device).

### Sous iOS
{: #on-ios }
Utilisez la procédure normale du système d'exploitation pour supprimer l'application.  
Appuyez sur **Install** pour installer la version antérieure de l'application. Suivez la procédure décrite dans [Installation d'une application sur un terminal iOS](#installing-an-application-on-an-ios-device).

### Sous Windows Phone
{: #on-windows-phone }
Appuyez sur **Revert**. Suivez la procédure décrite dans [Installation d'une application sur un terminal Windows Phone ](#installing-an-application-on-a-windows-phone-device).
...
Une application marquée comme favorite sur votre terminal indique que cette application vous intéresse. Cette application est ensuite répertoriée dans la liste des applications favorites pour faciliter sa localisation. Cette application est affichée sur chaque terminal qui vous appartient et qui est compatible avec l'application. Si une version ultérieure de l'application est disponible dans Application Center, l'application est répertoriée dans la vue **Updates**.
...
Vous pouvez noter une version d'application installée sur votre terminal mobile ; la revue doit inclure une évaluation et un commentaire. Vous ne pouvez soumettre une revue d'une version d'application que si cette version est installée sur votre terminal mobile.
...
2. Entrez un nombre d'étoiles non nul.
    ...
    Sur les terminaux mobiles dotés d'écrans tactiles, appuyez sur 1 à 5 étoiles pour représenter votre évaluation d'approbation de la version de l'application. Une étoile représente le plus faible niveau d'appréciation et cinq étoiles représentent le plus haut niveau d'appréciation.
...
Les revues sont affichées en ordre décroissant à partir de la revue la plus récente. Si le nombre de revues remplit plusieurs écrans, appuyez sur **Load more** pour afficher d'autres revues. Sur les terminaux Android, iOS et Windows Phone, les détails de la revue sont visibles dans la liste.
