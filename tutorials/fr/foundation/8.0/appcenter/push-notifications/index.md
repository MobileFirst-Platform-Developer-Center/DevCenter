...
2. Enregistrez ce compte en utilisant l'API Google dans la [console de l'API Google](https://code.google.com/apis/console/). La procédure d'enregistrement crée un projet par défaut que vous pouvez renommer. Le nom que vous donnez à ce projet GCM n'est pas lié au nom du module d'application Android. Lorsque le projet est créé, un ID de projet GCM est ajouté à la fin de l'URL du projet. Vous devez enregistrer ce numéro de fin en tant qu'ID de projet pour référence future.
