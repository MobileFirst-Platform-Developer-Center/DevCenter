...
### Android
{: #android }
Système d'exploitation mobile créé par Google, dont la plus grande partie est publiée sous les licences de code source ouvert Apache 2.0 et GPLv2. Voir aussi Terminal mobile.
...
### Application
{: #app }
Application Web ou pour terminal mobile. Voir aussi Application Web.
...
doit se trouver sur un terminal pour pouvoir installer des applications
...
{: #company-hub }
Application qui peut distribuer d'autres applications spécifiées devant être installées sur un terminal mobile. Par exemple, Application Center est un concentrateur d'entreprise. Voir également [Application Center](#application-center).
...
### Terminal
{: #device }
Voir [Terminal mobile](#mobile-device)

### Contexte de terminal
{: #device-context }
Données permettant d'identifier l'emplacement d'un terminal. Ces données peuvent inclure des coordonnées géographiques, des points d'accès Wi-Fi et des détails d'horodatage. Voir aussi Déclencheur.

### Enregistrement de terminal
{: #device-enrollment }
Processus consistant, pour un propriétaire, à enregistrer son terminal comme terminal autorisé.
...
### Passerelle
{: #gateway }
Terminal ou programme permettant de connecter des réseaux ou des systèmes à des architectures réseau différentes.
...
### Magasin local
{: #local-store }
Zone d'un terminal dans laquelle des applications peuvent stocker et extraire des données en local sans nécessiter de connexion réseau.
...
### Application native
{: #native-app }
Application compilée en code binaire pour utilisation sur le système d'exploitation mobile du terminal.
...
### Session
{: #sessions }
Connexion logique ou virtuelle entre deux postes, programmes logiciels ou terminaux sur un réseau, qui permet aux deux éléments de communiquer et d'échanger des données pour la durée de la session.
...
du code peut être développé pour un terminal Android sur un ordinateur,
...
### Message système
{: #system-message }
Message automatique sur un terminal mobile qui indique l'état opérationnel ou des alertes, par exemple si des connexions sont ou non établies.
...
Notification à destination des terminaux ayant souscrit à une balise spécifique. Des balises sont utilisées pour représenter des rubriques susceptibles d'intéresser un utilisateur. Voir aussi Notification de diffusion.
...
### Déclencheur
{: #trigger }
Mécanisme qui détecte une occurrence et qui peut, en réponse, enclencher un traitement supplémentaire. Des déclencheurs peuvent être activés lors de modifications du contexte de terminal. Voir aussi Contexte de terminal.
