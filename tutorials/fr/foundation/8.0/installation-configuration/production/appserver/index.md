...
<b>Remarque :</b> dans une topologie de parc de serveurs, vous ne pouvez pas utiliser le connecteur JMX RMI. Dans ce type de topologie, le connecteur SOAP seulement est pris en charge par {{ site.data.keys.product }}.</li>
                            ...
                            <li>Configurez autant de serveurs autonomes que vous voulez de membres dans le parc de serveurs.
                                <br/><br/>
                            Chacun de ces serveurs autonomes doit communiquer avec la même base de données. Vous devez vous assurer qu'aucun port utilisé par l'un de ces serveurs n'est utilisé par ailleurs par un autre serveur configuré sur le même hôte.
Cette contrainte concerne les ports utilisés par les protocoles HTTP, HTTPS, REST, SOAP et RMI. <br/><br/>
Le service d'administration de {{ site.data.keys.mf_server }}, le service Live Update de {{ site.data.keys.mf_server }} et un ou plusieurs environnements d'exécution de {{ site.data.keys.product_adj }} doivent être déployés sur chacun de ces serveurs. <br/><br/>
...
<br/><br/>
                            Pour activer la configuration de la base de données qui est utilisée par les composants {{ site.data.keys.mf_server }}, définissez les valeurs des propriétés suivantes :
                                <ul>
                            ...
                            <br/><br/>
                            Etant donné que les bases de données {{ site.data.keys.mf_server }} sont partagées entre les serveurs d'applications d'un parc de serveurs, il suffit d'exécuter ces deux commandes une seule fois, quel que soit le nombre de serveurs dans le parc de serveurs.
</li>
                            <li>Si vous voulez installer un autre environnement d'exécution, vous devez configurer une autre base de données avec un autre nom ou un autre schéma de base de données.
Pour ce faire, éditez le fichier Ant, modifiez les propriétés et exécutez la commande suivante une seule fois, quel que soit le nombre de serveurs dans le parc de serveurs : <code>rép_install_mfp/shortcuts/ant -f fichier-ant-création-base-données.xml rtmdatabases</code>.</li>
                        </ul>
                    </li>
...
<ul>
                            <li>Choisissez le fichier Ant qui correspond à votre serveur d'applications et à votre base de données dans le répertoire <b>rép\_install\_mfp/MobileFirstServer/configuration-samples</b> afin de déployer le service d'administration, le service Live Update et l'environnement d'exécution sur les serveurs. <br/><br/>
                            Par exemple, choisissez le fichier <b>configure-liberty-db2.xml</b> pour un déploiement sur le serveur Liberty avec la base de données DB2. Effectuez autant de copies de ce fichier que vous voulez de membres dans le parc de serveurs.
<br/><br/>
                            <b>Remarque :</b> conservez ces fichiers une fois la configuration terminée car vous pourrez les réutiliser pour la mise à niveau des composants {{ site.data.keys.mf_server }} déjà déployés, ou pour désinstaller les composants de chaque membre du parc de serveurs. </li>
...
<br/><br/>
                            Pour configurer le serveur comme membre du parc de serveurs, définissez les valeurs des propriétés suivantes :
                                <ul>
...
Il doit être identique pour tous les membres du parc de serveurs.
                                ...
                                <br/>
                                <b>Remarque :</b> si vous prévoyez d'installer plusieurs environnements d'exécution sur les serveurs du parc de serveurs, spécifiez l'attribut id et définissez une valeur unique pour chaque environnement d'exécution dans les tâches Ant <b>installmobilefirstruntime</b>, <b>updatemobilefirstruntime</b> et <b>uninstallmobilefirstruntime</b>.
                                <br/>
                                Exemple :
{% highlight xml %}
...
Cette contrainte concerne les ports utilisés par les protocoles HTTP, HTTPS, REST, SOAP et RMI. </li>
                    ...
                    <li>Configurez chaque serveur de façon appropriée pour le type de serveur d'applications.<br/>Vous devez définir certaines propriétés JNDI correctement. Dans une topologie de parc de serveurs, les valeurs des propriétés JNDI mfp.config.service.user et mfp.config.service.password doivent être identiques pour tous les membres du parc de serveurs. Pour Apache Tomcat, vous devez aussi vérifier que les arguments JVM sont définis correctement.
                       <ul>
                            <li><b>Profil Liberty de WebSphere Application Server</b>
                                      <br/>
                                      Dans le fichier server.xml, définissez les propriétés JNDI apparaissant dans l'exemple de code suivant :
{% highlight xml %}
...
{% endhighlight %}
                            ...
                            <li><b>Apache Tomcat</b>
                                <br/>
                                Modifiez le fichier <b>conf/server.xml</b> afin de définir les propriétés JNDI suivantes dans le contexte du service d'administration et dans le contexte de chaque environnement d'exécution :
{% highlight xml %}
...
{% endhighlight %}
                                La propriété <b>mfp.admin.serverid</b> doit avoir pour valeur l'identificateur que vous avez défini pour ce membre de parc de serveurs.
Cet identificateur doit être unique parmi tous les membres du parc de serveurs. <br/>
                                Veillez à ce que l'argument JVM <code>-Djava.rmi.server.hostname</code> ait pour valeur l'adresse IP ou le nom d'hôte qui est utilisé par les membres distants pour accéder à ce serveur. Vous ne pouvez donc pas indiquer <b>localhost</b>. De plus, assurez-vous que l'argument JVM <code>-Dcom.sun.management.jmxremote.port</code> ait pour valeur un port qui n'est pas déjà utilisé pour activer les connexions JMX RMI. Les deux arguments sont définis dans la variable d'environnement <b>CATALINA_OPTS</b>.
                            </li>
                            <li><b>Profil complet de WebSphere Application Server</b>
                                <br/>
                                Vous devez déclarer les propriétés JNDI suivantes dans le service d'administration et dans chaque application d'environnement d'exécution déployée sur le serveur :
<ul>
                                    <li><b>mfp.topology.clustermode</b></li>
...
<ul>
                            <li><b>Profil Liberty de WebSphere Application Server</b>
                                <br/>
                                Vous pouvez configurer le magasin de clés de confiance à l'aide d'utilitaires IBM tels que Keytool ou iKeyman.
                                <ul>
...
{% endhighlight %}
                            ...
                            <li><b>Profil complet de WebSphere Application Server</b>
                                <br/>
                                Configurez le magasin de clés de confiance dans la console d'administration WebSphere Application Server.
                                <ul>
