...
6. Déplacez le répertoire qui contient les serveurs vers un emplacement ne nécessitant pas de privilèges spécifiques. 
    ...
    * Dans le répertoire **etc**, créez un fichier **server.env** avec le contenu suivant : `WLP_USER_DIR=<chemin d'accès à un répertoire dans lequel tout utilisateur peut écrire des données>`. Par exemple, sous Windows : `WLP_USER_DIR=C:\LibertyServers\usr`.
