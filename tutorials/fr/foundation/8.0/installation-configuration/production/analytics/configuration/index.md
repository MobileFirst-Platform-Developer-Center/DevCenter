...
Dans tous les cas où la clé ne contient pas de point (par exemple
**httpport** et non **http.enabled**), le paramètre
peut être contrôlé par des variables d'environnement système où le nom de la variable est
préfixé avec **ANALYTICS_**. Lorsque la propriété JNDI et la
variable d'environnement système sont définies toutes les deux, la variable
d'environnement système est prioritaire. Par exemple, si la propriété JNDI
**analytics/httpport** et la variable d'environnement système
**ANALTYICS_httpport** sont définies toutes les deux, la valeur de
**ANALYTICS_httpport** est utilisée. 

#### Durée de vie des documents 
{: #document-time-to-live-ttl }
La durée de vie détermine l'établissement et la gestion d'une règle de conservation des données. Vos
décisions ont des conséquences notables sur vos besoins en matière de ressources
système. Plus vous conservez les données longtemps, plus vous aurez besoin de mémoire RAM
et de disque, ainsi que de procéder à une mise à l'échelle.
...
Etant donné qu'Elasticsearch est imbriqué dans
...
3. Examinez tous les indicateurs de configuration dans les scénarios de
