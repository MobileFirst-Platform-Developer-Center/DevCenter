...
| mfp.admin.jmx.user       | Obligatorio para el perfil de Liberty y para la granja de servidores de WebSphere Application Server, de lo contrario, opcional. | Nombre de usuario para la conexión REST de JMX. | Perfil de WebSphere Application Server Liberty: el nombre de usuario de la conexión JMX REST.<br/><br/>WebSphere Application Server Farm: el nombre de usuario de la conexión SOAP.<br/><br/>WebSphere Application Server Network Deployment: el nombre de usuario del administrador de WebSphere si el host virtual correlacionado con la aplicación de administración de {{ site.data.keys.mf_server }} no es el host predeterminado.<br/><br/>Colectivo de Liberty: el nombre de usuario del administrador del controlador definido en el elemento `<administrator-role>` del archivo server.xml del controlador de Liberty. |
| mfp.admin.jmx.pwd	| Obligatorio para el perfil de Liberty y para la granja de servidores de WebSphere Application Server, de lo contrario, opcional. | Contraseña de usuario para la conexión REST de JMX. | Perfil de WebSphere Application Server Liberty: la contraseña de usuario correspondiente a la conexión JMX REST.<br/><br/>WebSphere Application Server Farm: la contraseña de usuario de la conexión SOAP.<br/><br/>WebSphere Application Server Network Deployment: la contraseña de usuario del administrador de WebSphere si el host virtual correlacionado con la aplicación de administración del servidor de {{ site.data.keys.mf_server }} no es el host predeterminado.<br/><br/>Colectivo de Liberty: la contraseña del administrador del controlador que se ha definido en el elemento `<administrator-role>` del archivo server.xml del controlador de Liberty. |
...
|--------------------------|-----------------------|--------------|
| mfp.admin.actions.prepareTimeout | Opcional | Tiempo de espera en milisegundos para transferir datos desde el servicio de administración hasta el tiempo de ejecución durante una transacción de despliegue. Si no se puede acceder el entorno de ejecución en este periodo, se genera un error y la transacción de despliegue finaliza.<br/><br/>Valor predeterminado: 1800000 ms (30 mins) |
| mfp.admin.actions.commitRejectTimeout | Opcional | Tiempo de espera en milisegundos, cuando se establece contacto con un tiempo de ejecución, para confirmar o rechazar una transacción de despliegue. Si no se puede acceder el entorno de ejecución en este periodo, se genera un error y la transacción de despliegue finaliza.<br/><br/>Valor predeterminado: 120000 ms (2 mins) |
...
| mfp.admin.serverid | Obligatorio para granjas de servidores y colectivo de Liberty, de lo contrario, opcional. | Granja de servidores: El mismo identificador. Debe ser diferente para cada servidor de la granja de servidores.<br/><br/> Colectividad de Liberty: El valor debe ser controller. |
...
| mfp.admin.jmx.user | Opcional. WebSphere Application Server Farm: el nombre de usuario de la conexión SOAP.<br/><br/>Colectivo de Liberty: el nombre de usuario del administrador del controlador definido en el elemento `<administrator-role>` del archivo server.xml del controlador de Liberty. |
| mfp.admin.jmx.pwd | Opcional. WebSphere Application Server Farm: la contraseña de usuario de la conexión SOAP.<br/><br/>Colectivo de Liberty: la contraseña del administrador del controlador definido en el elemento `<administrator-role>` del archivo server.xml del controlador de Liberty. |
| mfp.admin.serverid | Obligatorio para granjas de servidores y colectivo de Liberty; opcional en caso contrario.<br/><br/>Granja de servidores: el identificador de servidor. Debe ser diferente para cada servidor de la granja de servidores.<br/><br/>Colectivo de Liberty: el identificador de miembro. El identificador debe ser distinto para cada miembro del colectivo. El valor controller no se puede utilizar puesto que está reservado para el controlador colectivo. |
...
| mfp.licenseTracking.enabled |Un valor utilizado para habilitar o inhabilitar el seguimiento de dispositivo en {{ site.data.keys.product }}.<br/><br/>Por motivos de rendimiento, puede inhabilitar el seguimiento de dispositivo cuando {{ site.data.keys.product }} se ejecuta solo en aplicaciones Business-to-Consumer (B2C). Cuando el seguimiento de dispositivo está inhabilitado, los informes de licencia también están inhabilitados y no se generan métricas de licencia.<br/><br/>Los valores posibles son true (valor predeterminado) y false.  |
