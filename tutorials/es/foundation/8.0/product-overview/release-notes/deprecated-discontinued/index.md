...
Para obtener más información sobre cómo utilizar Eclipse con un ID habilitado para Maven, consulte la <a href="{{site.baseurl}}/tutorials/en/foundation/8.0/adapters/developing-adapters/">Guía de aprendizaje de desarrollo de adaptadores en Eclipse</a>.</p>
        ...
        <p>Instale {{site.data.keys.mf_dev_kit_full }} para probar los adaptadores y las aplicaciones con {{site.data.keys.mf_server }}.
...
Utilice <code>MFP.Server.invokeHttp</code> para añadir señales de autenticación a la cabecera de la solicitud HTTP.
</p></td>
    </tr>
    <tr>
        <td>
...
### API de JavaScript
{: #javascript-apis }
En la V8.0 ya no se da soporte a estas API JavaScript que afectan a la interfaz de usuario.
Se pueden sustituir con plugins de Cordova de terceros que ya existan o creando plugins de Cordova personalizados.
...
| `WLDevice.getID(options)` | Utilice plugins de Cordova que proporcionen esta funcionalidad. **Nota:** `device.uuid` del plugin c**ordova-plugin-device** proporciona esta característica.  |
...
| `static WL.createInstance(actividad android.app.Activity)` | `static WL.createInstance(android.content.Context context)` crea una instancia compartida.  |
